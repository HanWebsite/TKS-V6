...
if (!target || !time || !thread || !proxyFile || !rps) {
 console.error('Error: Isi Dengan Bener:');
    console.error('  target: Target LU.');
    console.error('  time: Time Max (120).');
    console.error('  thread: MAX (1000).');
    console.error('  proxylist: PROXY PAKE LA.');
    console.error('  rps: Max 512');
...
// Validate RPS value
if (isNaN(rps) || rps <= 0) {
  console.error('RPS YG BENER ANJ');
  process.exit(1);
}
...
console.log(`SUCCESS: Attack sent successfully! | Target: ${target} | Time: ${time} seconds | Threads: ${thread} | Requests per second: ${rps} |`);
